...
from tgbot.services.sqlite_logic import get_preoder
...
def products_item_position_swipe_fp(remover, category_id):
    ...
    for count, a in enumerate(range(remover, len(get_positions))):
        if count < 10:
            get_items = get_itemsx(position_id=get_positions[a]['position_id'])
            len_count = f"{len(get_items)} шт"
            if len(get_items) == 0: #Проверка на доступность предзаказа
                result = get_preoder(get_positions[a]['position_id'])
                if result:
                    len_count = "Предзаказ"

            keyboard.add(ikb(
                f"{get_positions[a]['position_name']} | {get_positions[a]['position_price']}₽ | {len_count}",
