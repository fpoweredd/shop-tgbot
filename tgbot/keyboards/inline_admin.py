...
def add_pre_order():
    keyboard = InlineKeyboardMarkup(
        ...
        ikb("✅", callback_data=f"add_pre_opder:1"),
        ikb("❌", callback_data=f"add_pre_opder:0")
    )
...
def position_edit_open_finl(position_id, category_id, remover):
    keyboard = InlineKeyboardMarkup(
        ...
        ikb("🔴 Запретить предзаказ", callback_data=f"position_edit_preorder:{position_id}:{category_id}:{remover}:0"),
        ikb("🟢 Разрешить предзаказ", callback_data=f"position_edit_preorder:{position_id}:{category_id}:{remover}:1"),
    ).add(
