...
# Добавление категории
def add_positionx(position_name, position_price, position_description, position_photo, category_id, pre_order):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        con.execute("INSERT INTO storage_position "
                    "(position_name, position_price, position_description, position_photo, position_date, category_id, position_pre_order ) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    [position_name, position_price, position_description,
                     position_photo, get_date(), category_id, pre_order])
        con.commit()
        position_id = con.execute('SELECT position_id FROM storage_position').fetchall()
        return position_id[-1]['position_id']
...
def get_pre_orderx(**kwargs):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = f"SELECT * FROM storage_pre_order"
        sql, parameters = update_format_args(sql, kwargs)
        return con.execute(sql, parameters).fetchall()
...
def update_pre_order(position_id, pre_order):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.execute(f"UPDATE storage_position SET position_pre_order = ? WHERE position_id = ?", (pre_order, position_id))
        con.commit()
...
def create_dbx():
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        if len(con.execute("PRAGMA table_info(storage_promocode)").fetchall()) == 9:
        ...
        if len(con.execute("PRAGMA table_info(storage_pre_order)").fetchall()) == 6:
            print("DB was found(10/10)")
        else:
            con.execute("CREATE TABLE storage_pre_order("
                        "preorder_id INTEGER PRIMARY KEY AUTOINCREMENT,"
                        "user_id INTEGER,"
                        "username TEXT,"
                        "item_position_id INTEGER,"
                        "item_position_name TEXT,"
        ...
        # Создание БД с хранением данных платежных систем
        if len(con.execute("PRAGMA table_info(storage_payment_yoomoney)").fetchall()) == 4:
            print("DB was found(11/11)")
        else:
            con.execute("CREATE TABLE storage_payment_yoomoney("
                        "yoomoney_number INTEGER,"
                        "yoomoney_token TEXT,"
                        "yoomoney_client_id INTEGER,"
                        "way_form TEXT)")

            con.execute("INSERT INTO storage_payment_yoomoney("
                        "yoomoney_number, yoomoney_token, yoomoney_client_id, way_form) "
                        "VALUES (?, ?, ?, ?)",
                        ['None', 'None', 'None', 'False'])
            print("DB was not found(10/11) | Creating...")

        if len(con.execute("PRAGMA table_info(storage_payment_paymanual)").fetchall()) == 1:
            print("DB was found(12/12)")
        else:
            con.execute("CREATE TABLE storage_payment_paymanual("
                        "way_form TEXT)")

            con.execute("INSERT INTO storage_payment_paymanual("
                        "way_form) "
                        "VALUES (?)",
                        ['False',])
        ...
        if len(con.execute("PRAGMA table_info(storage_payment_payok)").fetchall()) == 5:
            print("DB was found(13/13)")
        else:
            con.execute("CREATE TABLE storage_payment_payok("
                        "api_key TEXT,"
                        "api_id INTEGER,"
                        "secret_key TEXT,"
                        "shop_id INTEGER,"
            ...
            con.execute("INSERT INTO storage_payment_payok("
                        "api_key, api_id, secret_key, shop_id,  way_form) "
                        "VALUES (?, ?, ?, ?, ?)",
                        ['None', 'None', 'None', 'None', 'False'])
            print("DB was not found(12/13) | Creating...")
