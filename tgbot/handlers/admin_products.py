...
from tgbot.keyboards.inline_admin import add_pre_order
from tgbot.keyboards.reply_all import finish_load_rep, items_frep
from tgbot.keyboards.inline_admin_preorder import preorder_load_items
...
from tgbot.services.sqlite_logic import get_len_items, update_pre_order_, delete_preoder, check_preorder
...
async def product_position_create_description(message: Message, state: FSMContext):
    try:
        if len(message.text) <= 600:
            ...
            await state.set_state("here_position_pre-order")
            await message.answer("<b>📁 Разрешить предзаказ на эту позицию? 💲</b>\n"
                                 "❕ Используйте клавиатуру ниже", reply_markup=add_pre_order()) #TODO здесь кнопки управления
        else:
...
@dp.callback_query_handler(IsAdmin(), text_startswith="add_pre_opder:", state="here_position_pre-order")
async def product_position_edit_category_open(call: CallbackQuery, state: FSMContext):
    select = int(call.data.split(":")[1])
    async with state.proxy() as data:
        data['pre-order'] = select

    await state.set_state("here_position_photo")
    await call.message.answer("<b>📁 Отправьте изображение для позиции 📸</b>\n"
                                "❕ Отправьте <code>0</code> чтобы пропустить.")
...
async def product_position_create_photo(message: Message, state: FSMContext):
    ...
    position_id = add_positionx(position_name, position_price, position_description, position_photo, category_id, pre_order)
...
@dp.callback_query_handler(IsAdmin(), text_startswith="position_edit_preorder:", state="*")
async def product_position_edit_preorder(call: CallbackQuery, state: FSMContext):
    position_id = call.data.split(":")[1]
    category_id = call.data.split(":")[2]
    remover = int(call.data.split(":")[3])
    select = int(call.data.split(":")[4])

    with suppress(MessageCantBeDeleted):
        await call.message.delete()
    update_pre_order(position_id, select)

    get_message, get_photo = get_position_admin(position_id)

    if get_photo is not None:
        await call.message.answer_photo(get_photo, get_message, reply_markup=position_edit_open_finl(position_id, category_id, remover))
    else:
        await call.message.answer(get_message, reply_markup=position_edit_open_finl(position_id, category_id, remover))
...
async def product_item_position_open(call: CallbackQuery, state: FSMContext):
    ...
    with suppress(MessageCantBeDeleted):
        await call.message.delete()
    get_pre_order = get_pre_orderx(item_position_id = position_id)
    count_all = []
    for i in get_pre_order:
        count_all.append(i['count'])
    async with state.proxy() as data:
        data['pre_order'] = count_all

    await state.set_state("here_add_items")
    await call.message.answer(f"На эту позицию оформлено <code>{sum(count_all)}</code> шт. предзаказов\n"
...
async def product_item_load_get(message: Message, state: FSMContext):
    ...
    await state.set_state("product_pre_order_load")
    await cache_msg.edit_text(f"<b>Желаете ль вы сразу выгрузить товары для <u>предзаказа</u></b>", reply_markup=preorder_load_items())


@rate_limit(0)
@dp.callback_query_handler(IsAdmin(), text_startswith="preorder_load_items:", state="product_pre_order_load")
async def product_pre_order_load(call: CallbackQuery, state: FSMContext):
    select = call.data.split(":")[1]
    result = False
    async with state.proxy() as data:
        category_id = data['here_cache_add_item_category_id']
        position_id = int(data['here_cache_add_item_position_id'])
        count_add = data['here_count_add_items']
        cache_msg = data['cache_msg']
        get_all_items = data['get_all_items']
    ...
    if select == "yes":
        if count_add >= sum(pre_order):
            date_count = sum(pre_order)

        else:
            date_count = count_add
        result = True
        get_positions = get_len_items(position_id)
    else:
        pass
    ...
    if result:
        user_id = None
        message_text = ''
        dase = get_pre_orderx(item_position_id = position_id)

        for row in get_pre_orderx(item_position_id = position_id):
            get_positions = get_len_items(position_id)
            message_text = ''
            user_send = True  # Если у человека на данную позицию еще есть предзаказы
            user_send = check_preorder(position_id, row['user_id'])
            for i in get_positions[:row['count']]:

                #TODO удаление товаров
                if user_send:
                    update_pre_order_(position_id, row['user_id'], date_count, i[0])
                    message_text += f"<code>{i[1]}</code>\n\n"
                else:
                    message_text = None

            if message_text != None:
                await bot.send_message(row['user_id'], f"<b>Вам пришли товары, которые вы предзаказали чуть раньше</b>\n\n{message_text}")
        #
        # # auto_load_pre_order(position_id, date_count, pre_order)
        # delete_preoder()
################################################################################################
####################################### УДАЛЕНИЕ ТОВАРОВ ######################################
